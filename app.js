...
const coordsValidationMiddleware = require("./middlewares/coordsValidationMiddleware");
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!templateId) {
        return res
          .status(404)
          .json({ error: "Id dont mach with any template" });
      }
      ...
      const templatePath = path.join(
        __dirname,
        `clients/${client}/templates/${templateConfig.htmlTemplateUrl}`
      );
      ...
      if (templateConfig.variables && templateConfig.variables.length > 0) {
        templateConfig.variables.forEach((variableKey) => {
          ...
          cc = cc.replace(`{{${variableKey}}}`, req.body[variableKey] ?? "");
        });
      }
...
app.get("/:client/coords", authMiddleware, async (req, res) => {
  const { client } = req.params;
  const clientDir = path.join(__dirname, "clients", client);
  const coordsPath = path.join(clientDir, "coords.json");

  if (!fs.existsSync(coordsPath)) {
    console.error(`Coordinate card no found for client: ${client}`);
    throw new Error(`Coordinate card no found for client: ${client}`);
  }

  const coordinates = JSON.parse(fs.readFileSync(coordsPath, 'utf8'));
  res.status(200).json(coordinates.filter(c=>!c.used));
});
